...
def _get_scan_value():
    """
    Runs scan_program.py or scan_program.exe if present; otherwise returns random.
    """
...
class PiCameraLiveFeed(tk.Frame):
    def __init__(self, parent, size=(960, 540), fps=20):
        ...
        self.size = size
        self.delay_ms = max(1, int(1000 / max(1, fps)))

        self.label = tk.Label(
            self,
            bg="black",
            fg="white",
            text="No camera selected.\nChoose a camera and click Connect.",
            justify="center"
        )
    ...
    def connect(self, camera_index: int = 0) -> bool:
        ...
        try:
            ...
            self.label.config(
                image="",
                text=f"Camera not available.\n\n{e}\n\nSelect a camera and try again.",
                justify="center"
            )
            return False
    ...
    def _tick(self):
        ...
        try:
            frame = self.picam2.capture_array()  # RGB numpy array
...
class HomePage(tk.Frame):
    def __init__(self, parent, controller: App):
        ...
        tk.Button(
            btn_frame, text="NEW BATCH", command=self.new_batch_flow,
            bg="Grey", fg="white", font=("Arial", 12), width=16
        ).grid(row=0, column=0, padx=10)

        tk.Button(
            btn_frame, text="EXISTING BATCH", command=self.select_existing_batch,
            bg="Grey", fg="white", font=("Arial", 12), width=16
        ).grid(row=0, column=1, padx=10)

        tk.Button(
            btn_frame, text="COMPARE", command=self.compare_batches_flow,
            bg="Grey", fg="white", font=("Arial", 12), width=16
...
class ScanPage(tk.Frame):
    def __init__(self, parent, controller: App):
        ...
        tk.Label(top, text="Scanning", font=("Segoe UI", 14, "bold")).pack(side=tk.LEFT, padx=10)
        ...
        tk.Label(cam_bar, text="Camera:").pack(side=tk.LEFT)
        ...
        tk.Button(cam_bar, text="Rescan", command=self.refresh_camera_list).pack(side=tk.LEFT, padx=4)
    ...
    def connect_camera(self):
        ...
        if idx is None:
            messagebox.showwarning("No camera", "No camera selected.", parent=self)
            return

        ok = self.live.connect(camera_index=idx)
        if ok:
    ...
    def refresh_table_and_plot(self):
        ...
        if vals:
            ...
            self.ax.text(0.5, 0.5, "No numeric scan values yet", ha="center", va="center",
    ...
    def do_scan(self):
        ...
        if ok:
            self.status_label.config(
                text=f"Last scan: {scan_val} at {datetime.now().strftime('%H:%M:%S')} • Saved • File: {self.batch_file.name}"
            )
...
class ComparePage(tk.Frame):
    def __init__(self, parent, controller: App):
        ...
        self.avg_batch_label = tk.Label(avg_frame, text="Per-batch averages will appear here.",
                                        justify="left", anchor="w")
        self.avg_batch_label.pack(fill=tk.X)

        self.avg_combined_label = tk.Label(avg_frame, text="Combined average: —",
                                           justify="left", anchor="w")
    ...
    def load_all(self):
        ...
        for f in self.batch_files:
            b = _infer_batch_str_from_file(f)
            lines.append(f"{b}: {avg_map[b]:.3f}" if b in avg_map else f"{b}: —")
...
if __name__ == "__main__":
    ...
    def _on_close():
        try:
            if app.current_page == "ScanPage":
                page = app.pages["ScanPage"]
